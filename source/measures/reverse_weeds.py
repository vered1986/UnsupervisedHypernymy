...
def main():
    """
    Reversed Weeds Precision.
    """

    # Get the arguments
    args = docopt("""Compute reversed Weeds Precision for a list of (x, y) pairs and save their scores.

    Usage:
        reverse_weeds.py <testset_file> <dsm_prefix> <output_file>
        ...
        <output_file> = where to save the results: a tab separated file with x\ty\tlabel\trelation\tscore,
                        where the score is reversed Weeds Precision (for y as the hypernym of x).
    """)
    ...
    with codecs.open(output_file, 'w', 'utf-8') as f_out:
        ...
        for (x, y, label, relation) in test_set:
            ...
            if x_index > -1 and y_index > -1:
                ...
                score = reversed_weeds_prec(x_row, y_row)

            print >> f_out, '\t'.join((x, y, label, '%.5f' % score))


def reversed_weeds_prec(x_row, y_row):
    """
    ...
    x_row.to_ones()
    numerator = y_row.multiply(x_row).sum() # dot-product

    # The sum of x's contexts (for ppmi) is the sum of x_row.
    denominator = y_row.sum()
