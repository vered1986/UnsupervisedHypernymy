...
def main():
    ...
    # Get the arguments
    args = docopt("""Compute the revered clarkeDE for a list of (x, y) pairs and save their scores.

    Usage:
        reverse_clarkeDE.py <testset_file> <dsm_prefix> <output_file>
        ...
        <output_file> = where to save the results: a tab separated file with x\ty\tlabel\trelation\tscore,
                        where the score is reversed ClarkeDE (for y as the hypernym of x).
    """)
    ...
    with codecs.open(output_file, 'w', 'utf-8') as f_out:
        ...
        for (x, y, label, relation) in test_set:
            ...
            if x_index > -1 and y_index > -1:
                ...
                score = reversed_clarkeDE(x_row, y_row)

            print >> f_out, '\t'.join((x, y, label, '%.5f' % score))


def reversed_clarkeDE(x_row, y_row):
    """
